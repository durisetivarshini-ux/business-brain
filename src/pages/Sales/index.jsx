...
function NewDealModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    toast.success(`Deal for ${form.client} created successfully!`);
    setLoading(false);
    onClose();
  };
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0B1120] shadow-2xl p-8 relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-[#10B981]/50 transition-colors"
          ...
          <div>
            ...
            <select
              className="w-full px-4 py-3 rounded-xl bg-[#050816] border border-white/10 text-white text-sm focus:outline-none focus:border-[#10B981]/50 transition-colors"
              value={form.stage}
              onChange={e => setForm({ ...form, stage: e.target.value })}
            >
              {['Prospecting', 'Proposal', 'Negotiation', 'Closing', 'Won'].map(s => (
          ...
          <div className="flex gap-3 mt-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-bold hover:bg-white/10 transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={loading} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-[#10B981] to-[#00D4FF] text-white text-sm font-bold shadow-lg disabled:opacity-60 transition-all hover:scale-[1.02]">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(5,8,22,0.85)', backdropFilter: 'blur(8px)' }}>
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0B1120] shadow-2xl p-8 relative">
        ...
        <button onClick={onClose} className="w-full mt-6 py-3 rounded-xl bg-gradient-to-r from-[#10B981] to-[#00D4FF] text-white text-sm font-bold shadow-lg hover:scale-[1.02] transition-all">
          Close
        </button>
      </div>
    </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-[#10B981] to-[#00D4FF] text-white text-sm font-bold shadow-[0_0_15px_rgba(16,185,129,0.4)] transition-transform hover:scale-[1.02]"
          >
            + New Deal
          </button>
        </div>
